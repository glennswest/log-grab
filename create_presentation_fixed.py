...
def apply_red_hat_styling(shape, colors, is_title=False):
    """Apply Red Hat styling to text shapes."""
...
def format_bullet_text(text, max_line_length=65):
    """Format text to prevent overflow by breaking long lines."""
    lines = text.split('\n')
    formatted_lines = []
...
comprehensive pod failure detection""", colors)
    
    # Slide 6: Development Phase 2
    create_slide(prs, 1, "Development Phase 2: GUI Enhancement", """Claude Request 2: "Create a separate python script, that acts as a user 
interface for the logs, allowing navigation of multiple projects, 
multiple pods per project, and multiple logs. Use Tkinter"

What Claude Built:
• Complete Tkinter GUI application with hierarchical navigation
• Search and filtering capabilities with result highlighting
...
• Enhanced syntax highlighting specifically for watcher log entries
...
Key Innovation: Proactively handled breaking changes and created 
comprehensive setup automation""", colors)
...
Key Innovation: Interpreted user intent and created comprehensive 
environment management system""", colors)
...
Key Innovation: Diagnosed root cause and built enterprise-grade 
authentication resilience""", colors)
    
    # Slide 11: Technical Innovations
    create_slide(prs, 1, "Key Technical Innovations", """Authentication Resilience:
...
PYTHON_CANDIDATES = [
    ...
    "python3.11", "python3.12", "python3.13"  # Version-specific
]""", colors)
    
    # Slide 12: GUI Features
...
• Horizontal scaling for high-volume environments""", colors)
    ...
    # Slide 16: AI Development Lessons
